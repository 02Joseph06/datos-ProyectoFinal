...
# Estado resuelto indexado como tupla 1D de 54 elementos (optimizado para sets y comprobaciones veloces).
ESTADO_RESUELTO = (
    'W', 'W', 'W', 'W', 'W', 'W', 'W', 'W', 'W',  
    'G', 'G', 'G', 'G', 'G', 'G', 'G', 'G', 'G',  
    'R', 'R', 'R', 'R', 'R', 'R', 'R', 'R', 'R',  
...
# Historial de todos los giros matematicos posibles (18 en total).
MOVIMIENTOS = [
...
# Nodo para algoritmo BFS. Almacena el padre para poder reconstruir los pasos.
class Nodo:
...
# Rotacion de 90 grados de una cara especifica (Matriz 1D). 
# Reordena los indices fijamente para simular el giro fisico sin requerir complejas matrices 2D o 3D.
...
# Simulador matematico de los giros. Extrae los 12 bloques adyacentes al corte girado (4 grupos de 3, ej: idx_L, idx_F...).
# Realiza un desplazamiento ciclico permutando sus valores y luego rota su cara pertinente.
...
# Evalua los estados mediante un arbol, nivel por nivel usando una cola FIFO (First-In, First-Out).
# Esto nos garantiza matematicamente que la primera solucion encontrada siempre sera la mas corta y rapida posible.
def bfs_resolver_rubik(estado_inicial, movimientos_validos=None):
    ...
    visitados = set([estado_inicial]) # HashSet para descate de ramas muertas o bucles infinitos en tiempo O(1).
    
    nodos_expl = 0
    LIMITE_SEGURO_NODOS = 100000 # Freno de emergencia (Crash prevention).
